...
static uint32_t in_room(dungeon_t *d, int16_t y, int16_t x) //returns 1 if the x and y are in a room I think
{
  int i;
...
static uint32_t adjacent_to_room(dungeon_t *d, int16_t y, int16_t x) //returns 1 if adjacent to room
{
...
static uint32_t is_open_space(dungeon_t *d, int16_t y, int16_t x) //yep
{
  return !hardnessxy(x, y);
}

static int32_t corridor_path_cmp(const void *key, const void *with) { //no idea
  return ((corridor_path_t *) key)->cost - ((corridor_path_t *) with)->cost;
}

static void shortest_path(dungeon_t *d, corridor_path_t path[DUNGEON_Y][DUNGEON_X], corridor_path_t *p)
{
...
static void shortest_path_dig(dungeon_t *d, corridor_path_t path[DUNGEON_Y][DUNGEON_X], corridor_path_t *p)
{
  static uint32_t initialized = 0;
  heap_t h;
  uint32_t x, y;

  if (!initialized) { //everything set to 0
    for (y = 0; y < DUNGEON_Y; y++) {
      for (x = 0; x < DUNGEON_X; x++) {
        path[y][x].pos[dim_y] = y;
        path[y][x].pos[dim_x] = x;
...
static int connect_two_rooms(dungeon_t *d, room_t *r1, room_t *r2) //makes the corridors (paths)
{
  pair_t e1, e2;
...
static int connect_rooms(dungeon_t *d) //just calles connect_two_rooms for the rooms
{
  uint32_t i;
...
static int smooth_hardness(dungeon_t *d) //creates the hardness of the dungeon
{
  int32_t i, x, y;
  int32_t s, t, p, q;
...
static int empty_dungeon(dungeon_t *d) //init empty dungeon
{
  uint8_t x, y;
...
static int place_rooms(dungeon_t *d) //init rooms
{
  pair_t p;
  uint32_t i;
  int success;
...
static void place_stairs(dungeon_t *d) //random stair placement
{
  pair_t p;
  do {
...
static int make_rooms(dungeon_t *d) //creates all the rooms sizes
{
  uint32_t i;
...
int gen_dungeon(dungeon_t *d) //calls all the functions to create the dungeon
{
  empty_dungeon(d);
...
void render_dungeon(dungeon_t *d) //prints to console
{
  pair_t p;
...
int write_dungeon_map(dungeon_t *d, FILE *f) //save dungeon hardness
{
  uint32_t x, y;
...
int write_rooms(dungeon_t *d, FILE *f) //save dungeon rooms
{
  uint32_t i;
  uint16_t p;
...
uint16_t count_up_stairs(dungeon_t *d) //returns num of upward stairs
{
  uint32_t x, y;
  uint16_t i;
...
uint16_t count_down_stairs(dungeon_t *d) //returns num of downward stairs
{
  uint32_t x, y;
  uint16_t i;
...
int write_stairs(dungeon_t *d, FILE *f) //save dungeon stairs
{
  uint16_t num_stairs;
  uint8_t x, y;
...
uint32_t calculate_dungeon_size(dungeon_t *d) //save file size
{
...
int makedirectory(char *dir) //makes a folder for storing the dungeon save in the home dir
{
  char *slash;
...
int write_dungeon(dungeon_t *d, char *file) //calles all the write functions to save
{
  char *home;
  char *filename;
  FILE *f;
...
int read_dungeon_map(dungeon_t *d, FILE *f) //loads hardness
{
  uint32_t x, y;
...
int read_stairs(dungeon_t *d, FILE *f) //loads stairs
{
  uint16_t num_stairs;
  uint8_t x, y;
...
int read_rooms(dungeon_t *d, FILE *f) //loads rooms
{
  uint32_t i;
  uint32_t x, y;
  uint16_t p;
...
int read_dungeon(dungeon_t *d, char *file) //called all read functions
{
...
void usage(char *name) //shows user the load and save options
{
  fprintf(stderr,
...
/* void queue(dungeon_t *d, char display[80][21], int x, int y, int num)
{
	display[x][y] = num;
	...
	if( ((d->map[x-1][y] == ter_wall_immutable) || (d->map[x-1][y] == ter_wall)) &&  (display[x][y] == -1) ) //left
	{
		queue(d, display, x-1, y, num);
	}
	
	if( ((d->map[x-1][y-1] == ter_wall_immutable) || (d->map[x-1][y-1] == ter_wall)) &&  (display[x][y] == -1) ) //top left
	{
		queue(d, display, x-1, y-1, num);
	}

	if( ((d->map[x][y-1] == ter_wall_immutable) || (d->map[x][y-1] == ter_wall)) &&  (display[x][y] == -1) ) //top
	{
		queue(d, display, x, y-1, num);
	}

	if( ((d->map[x+1][y-1] == ter_wall_immutable) || (d->map[x+1][y-1] == ter_wall)) &&  (display[x][y] == -1) ) //top right
	{
		queue(d, display, x+1, y-1, num);
	}
	
	if( ((d->map[x+1][y] == ter_wall_immutable) || (d->map[x+1][y] == ter_wall)) &&  (display[x][y] == -1) ) //right
	{
		queue(d, display, x+1, y, num);
	}
	
	if( ((d->map[x+1][y+1] == ter_wall_immutable) || (d->map[x+1][y+1] == ter_wall)) &&  (display[x][y] == -1) ) //bottom right
	{
		queue(d, display, x+1, y+1, num);
	}
	
	if( ((d->map[x][y+1] == ter_wall_immutable) || (d->map[x][y+1] == ter_wall)) &&  (display[x][y] == -1) ) //bottom
	{
		queue(d, display, x, y+1, num);
	}
	
	if( ((d->map[x-1][y+1] == ter_wall_immutable) || (d->map[x-1][y+1] == ter_wall)) &&  (display[x][y] == -1) ) //bottom left
	{
		queue(d, display, x-1, y+1, num);
	}
}
...
int main(int argc, char *argv[])
{
 ...
 if (argc > 1) { //if only 1 argument
  ...
  if (!do_load) { //generate a dungeon
    /* Set a valid position for the PC */
	int placePC = 0;
	
	while(placePC == 0)
	{
		int placeY = (rand() % 18) + 1;
		int placeX = (rand() % 78) + 1;
		if(d.map[placeY][placeX] == ter_floor_room)
		{
			d.pc[dim_x] = placeX;
			d.pc[dim_y] = placeY;
			placePC = 1;
  ...
  if (do_save) { //save the generated dungeon
    if (do_save_seed) {
  ...
  static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p; 
  shortest_path(&d, path, p);
  for(int i = 0; i < 21; i++) //prints shortest path
  {
	  for(int j = 0; j < 80; j++)
	  {
		  if(path[i][j].cost == 0)
  ...
  static corridor_path_t path1[DUNGEON_Y][DUNGEON_X], *p1;
  shortest_path_dig(&d, path1, p1);
    for(int i = 0; i < 21; i++) //prints digging shortest path
  {
	  for(int j = 0; j < 80; j++)
	  {
		  if(path[i][j].cost == 0)
		  {
			  printf("@");
		  }else if(path1[i][j].cost != INT_MAX)
		  {
			  printf("%d", path1[i][j].cost % 10);
		  }else 
		  {
			  printf(" ");
		  }
